"""
Data Analyst Telegram Bot powered by Claude AI
==============================================
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• Request charts: _'plot revenue over time'_",
        parse_mode="Markdown",
    )
...
async def cmd_clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_state[chat_id] = {"filepath": None, "filename": None, "history": []}
    await update.message.reply_text("🗑️ Data and history cleared. Upload a new file to start fresh.")
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (fname.endswith(".csv") or fname.endswith(".xlsx") or fname.endswith(".xls")):
        await update.message.reply_text("⚠️ Please upload a *CSV* or *Excel* (.xlsx/.xls) file.", parse_mode="Markdown")
        return

    await update.message.reply_text("📥 Downloading your file…")
    tg_file = await context.bot.get_file(doc.file_id)
    save_path = f"/tmp/{chat_id}_{fname}"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Route text messages to the Claude agent."""
    chat_id = update.effective_chat.id
    user_text = update.message.text.strip()
    state = get_state(chat_id)

    if not state["filepath"]:
        await update.message.reply_text("📂 Please upload a CSV or Excel file first, then ask your question.")
        return
